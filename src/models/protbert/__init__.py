"""ProtBERT Model Wrapper for the Protein Foundation Model Benchmark Framework.

Wrapper for ProtBERT (Rostlab/prot_bert) using HuggingFace Transformers.
"""
...
class ProtBERT(BaseProteinModel):
    """Wrapper for ProtBERT model from Rostlab.

    ProtBERT is a BERT model trained on UniRef100/BFD protein sequences.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        config = config or {}
        config.setdefault("model_type", "protbert")
        ...
        self._model: Optional[BertModel] = None
        self._tokenizer: Optional[BertTokenizer] = None

        logger.info(f"ProtBERT wrapper initialized for {model_name}")

    def _load_model(self) -> None:
        """Load the ProtBERT model and tokenizer."""
        try:
            self._model = BertModel.from_pretrained(self.model_name).to(self.device)
            self._tokenizer = BertTokenizer.from_pretrained(self.model_name)
            self._model.eval()
            logger.info(f"Loaded ProtBERT from {self.model_name}")
        except Exception as e:
            logger.error(f"Failed to load ProtBERT: {e}")
            raise
    ...
    def extract_embeddings(
        ...
        """Extract embeddings from ProtBERT.
    ...
    def get_num_layers(self) -> int:
        return 30  # ProtBERT has 30 layers
    ...
    def save_pretrained(self, path: Union[str, Path]) -> None:
        ...
        logger.info(f"Saved ProtBERT to {path}")

    @classmethod
    def from_pretrained(cls, model_name: str = "Rostlab/prot_bert", **kwargs) -> "ProtBERT":
        """Load pretrained ProtBERT."""
